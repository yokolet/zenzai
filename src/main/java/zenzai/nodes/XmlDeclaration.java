...
public class XmlDeclaration extends LeafNode {
    ...
    public String getWholeDeclaration() {
        ...
        getWholeDeclaration(QuietAppendable.wrap(sb), new zenzai.nodes.Document.OutputSettings());
        return StringUtil.releaseBuilder(sb).trim();
    }

    private void getWholeDeclaration(QuietAppendable accum, zenzai.nodes.Document.OutputSettings out) {
        for (Attribute attribute : attributes()) {
            ...
            if (!key.equals(nodeName())) { // skips coreValue (name)
                ...
                if (!val.isEmpty()) {
                    accum.append("=\"");
                    zenzai.nodes.Entities.escape(accum, val, out, Entities.ForAttribute);
                    accum.append('"');
                }
            }
        }
    ...
    @Override
    void outerHtmlHead(QuietAppendable accum, zenzai.nodes.Document.OutputSettings out) {
        accum
                .append("<")
    ...
    @Override
    public zenzai.nodes.XmlDeclaration clone() {
        return (zenzai.nodes.XmlDeclaration) super.clone();
    }
}
