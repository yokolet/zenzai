...
public class NodeIterator<T extends zenzai.nodes.Node> implements Iterator<T>{
    private zenzai.nodes.Node root;         // root / starting node
    private @Nullable T next;               // the next node to return
    private zenzai.nodes.Node current;      // the current (last emitted) node
    private zenzai.nodes.Node previous;     // the previously emitted node; used to recover from structural changes
     ...
     */
    public void restart(zenzai.nodes.Node start) {
    ...
    private @Nullable T findNextNode() {
        zenzai.nodes.Node node = current;
        while (true) {
